 ...
 *
 * Single-file React component. Styling is Tailwind utility classes plus a small
 * injected <style> block for the keyframe / gradient-text bits Tailwind can't do
 * inline. No third-party runtime deps: entrance + scroll-reveal animation is
 * pure CSS driven by an IntersectionObserver (Framer Motion is not required).
 *
 * Assumes Tailwind is available in the host app. Inter + JetBrains Mono are
 * pulled from Google Fonts by the injected stylesheet; swap the four URL
 * constants below for the real deploy targets before shipping.
 */
...
/* ------------------------------------------------------------------- data --- */
// Every figure below is parsed from the agent's own logs + the Alpaca account
// snapshot for the competition session. Nothing here is illustrative.
...
function DebateCard({ side, role, accent, points }) {
  const bar = accent === "gain" ? "border-l-[#4ADE80]/70" : "border-l-[#F87171]/70";
    ...
    <div className="va-root relative isolate min-h-screen overflow-x-hidden bg-[#06030B] text-slate-200 antialiased">
      ...
      >
        ...
        <span className="va-hero-in mb-6 inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/[0.03] px-3 py-1 text-xs text-slate-400">
          <span
            className="h-1.5 w-1.5 rounded-full bg-[#F5A623]"
            aria-hidden
          />
      ...
      <section className="relative z-10 mx-auto max-w-6xl px-5 py-20">
        <Reveal>
          <SectionHeading kicker="The session" title="One run, on a paper account">
            ...
            Alpaca account snapshot. Nothing on this page is illustrative.
          </SectionHeading>
        </Reveal>
